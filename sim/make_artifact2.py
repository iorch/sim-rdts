...
<div class="wrap">
  ...
  hashpower en 5 nodos) vs <b>Knots disperso</b> en muchos nodos chicos (22% en 16). Vamos
  agregando mineros Knots grandes y medimos cuándo <b>gana el softfork</b> — cuando Core
  reorganiza a la cadena limpia y descarta sus bloques con datos.</p>
  <div class="versions">
  ...
  <div class="card">
    <h2>P(el softfork gana) vs hashpower de Knots</h2>
    <p class="sub">"Gana" = las cadenas convergen (Core adopta la limpia; el spam queda huérfano).
    La línea vertical marca 50% del hashpower. El eje X es <b>share de hashpower</b>, no número de nodos.</p>
    <div class="chart-wrap" id="w1">
      <svg id="c1" viewBox="0 0 800 360" preserveAspectRatio="xMidYMid meet"></svg>
      <div class="tooltip" id="t1"></div>
    </div>
    <div class="legend">
      <span><span class="swatch" style="background:var(--win)"></span>P(gana softfork)</span>
  ...
  <div class="card">
    ...
    <p class="sub">Cada reorg = la cadena de Core (con datos) reemplazada por la limpia de Knots
    → Core pierde esos bloques. En tiempo real (1 bloque ≈ 10 min, 144/día). Cuando pasa
  ...
  <div class="card">
    ...
    <p class="sub">21 nodos, 100% del hashpower. Se van sumando mineros Knots grandes (raw
    10, 15, … 85) hasta que Knots supera a Core.</p>
    <div class="dist">
  ...
  <div class="card">
    ...
    <table><thead><tr>
      <th>Share Knots</th><th>Nodos</th><th class="num">Gana softfork</th>
      <th class="num">P(gana)</th><th class="num">Bloques tirados</th>
      <th class="num">Reemplazos/día</th><th class="num">P(≥1/día)</th><th class="num">Prof. fork</th>
...
function drawWin(){
  ...
  const xl=E('text',{x:mL+iw/2,y:H-3,'text-anchor':'middle',fill:muted,'font-size':12});xl.textContent='Share de hashpower de Knots';svg.appendChild(xl);
  ...
  svg.addEventListener('pointermove',ev=>{const rc=svg.getBoundingClientRect();const px=(ev.clientX-rc.left)/rc.width*W;
    ...
    tt.innerHTML=`<div class="tt-n">${(best.knots_share*100).toFixed(0)}% Knots · ${(best.core_share*100).toFixed(0)}% Core</div>`+
      ...
      `<div class="tt-row"><span>P(gana)</span><b>${(best.p_win*100).toFixed(0)}%</b></div>`+
      `<div class="tt-row"><span>Gana</span><b>${best.wins}/${best.runs}</b></div>`+
      `<div class="tt-row"><span>Bloques tirados</span><b>${best.disc_mean}</b></div>`;
...
function drawIncentive(){
  ...
  const xs=s=>mL+((s-XMIN)/(XMAX-XMIN))*iw;
  const LO=0.1,HI=20;                       // eje Y logarítmico (reorgs/día)
  ...
  const lab=E('text',{x:W-mR,y:ys(1)-6,'text-anchor':'end',fill:disc,'font-size':11});lab.textContent='≥ 1 reemplazo/día → incentivo a señalar RDTS';svg.appendChild(lab);
  svg.appendChild(E('line',{x1:xs(50),y1:mT,x2:xs(50),y2:mT+ih,stroke:ref,'stroke-width':1.5,'stroke-dasharray':'5 4'}));
  for(let s=20;s<=90;s+=10){const t=E('text',{x:xs(s),y:H-mB+20,'text-anchor':'middle',fill:muted,'font-size':11,'font-family':'ui-monospace,Menlo,monospace'});t.textContent=s+'%';svg.appendChild(t);}
  const xl=E('text',{x:mL+iw/2,y:H-3,'text-anchor':'middle',fill:muted,'font-size':12});xl.textContent='Share de hashpower de Knots';svg.appendChild(xl);
  ...
  svg.addEventListener('pointermove',ev=>{const rc=svg.getBoundingClientRect();const px=(ev.clientX-rc.left)/rc.width*W;
    ...
    tt.innerHTML=`<div class="tt-n">${(best.knots_share*100).toFixed(0)}% Knots</div>`+
      `<div class="tt-row"><span>Reemplazos/día</span><b>${best.reorgs_day}</b></div>`+
      `<div class="tt-row"><span>P(≥1/día)</span><b>${(best.p_ge1_day*100).toFixed(0)}%</b></div>`+
      `<div class="tt-row"><span>Gana softfork</span><b>${(best.p_win*100).toFixed(0)}%</b></div>`;
